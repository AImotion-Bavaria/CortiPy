...
as JSON under `experiments/experiment3/results/`.
...
_D1_SPEC_PATH = (
    ...
    / "datasets"
    / "D1_random_streaming_matrices"
    / "datasets.json"
)


def _load_d1_specs() -> Sequence[dict[str, Any]]:
    try:
        data = json.loads(_D1_SPEC_PATH.read_text(encoding="utf-8"))
...
_FALLBACK_D1_SPECS: Sequence[dict[str, Any]] = (
    {
        "dataset_id": "D1",
        "label": "rand1",
        "kind": "synthetic",
...
SYNTHETIC_DATASETS: Sequence[dict[str, Any]] = tuple(_load_d1_specs()) or _FALLBACK_D1_SPECS
...
def _emit_paper_table(results: List[Dict[str, Any]], out_root: Path) -> None:
    """Paper-facing summary table for D1 single-channel full reads (console + PDF)."""
    out_root.mkdir(parents=True, exist_ok=True)

    d1_labels = [cfg.get("label") for cfg in DATASETS if isinstance(cfg, dict)]
    d1_labels = [x for x in d1_labels if isinstance(x, str)]
    combos = [("sbids", "parquet"), ("sbids", "edf"), ("bids", "edf")]

    def _mean_ms(dataset: str, container: str, fmt: str) -> float | None:
        for e in results:
            if e.get("dataset") != dataset or e.get("container") != container or e.get("format") != fmt:
                continue
            sm = e.get("stream_metrics") or {}
            block = sm.get("single_channel_full") if isinstance(sm, dict) else None
            if not isinstance(block, dict):
                return None
            times = block.get("times")
            if isinstance(times, list) and times and all(isinstance(t, (int, float)) for t in times):
                return float(sum(times) / len(times)) * 1000.0
            mean_s = block.get("mean")
            return float(mean_s) * 1000.0 if isinstance(mean_s, (int, float)) else None
        return None

    rows: list[list[str]] = []
    for container, fmt in combos:
        per_ds = [_mean_ms(ds, container, fmt) for ds in d1_labels]
        per_ds = [v for v in per_ds if isinstance(v, (int, float))]
        mean_ms = (sum(per_ds) / len(per_ds)) if per_ds else float("nan")
        rows.append([f"{container.upper()}+{fmt.upper()}", f"{mean_ms:.3f}"])

    # Compute EDF baseline as mean(SBIDS+EDF, BIDS+EDF) and speedup vs SBIDS+Parquet.
    ...
    parquet_ms = _find("SBIDS+PARQUET")
    sbids_edf_ms = _find("SBIDS+EDF")
    bids_edf_ms = _find("BIDS+EDF")
    edf_baseline_ms = None
    if sbids_edf_ms is not None and bids_edf_ms is not None:
        edf_baseline_ms = 0.5 * (sbids_edf_ms + bids_edf_ms)
    speedup = (edf_baseline_ms / parquet_ms) if (edf_baseline_ms and parquet_ms) else None

    # Console output
    print("\nD1 single-channel streaming read latency (mean, full channel)")
    print("Configuration\tMean (ms)")
    for cfg, mean_ms in rows:
        print(f"{cfg}\t{mean_ms}")
    if speedup is not None:
        print(f"\nSpeedup (EDF baseline / SBIDS+Parquet): {speedup:.2f}×")
    if sbids_edf_ms is not None and bids_edf_ms is not None:
        delta = abs(sbids_edf_ms - bids_edf_ms)
        print(f"SBIDS+EDF vs BIDS+EDF delta: {delta:.3f} ms (metadata overhead negligible for this workload)")
    ...
    table = ax.table(
        cellText=rows,
        colLabels=["Configuration", "Mean single-channel read latency (ms)"],
        cellLoc="center",
        loc="center",
    )
    ...
    title = "D1 – Mean single-channel streaming read latency"
    subtitle_parts = []
    if speedup is not None:
        subtitle_parts.append(f"Speedup (EDF baseline / Parquet): {speedup:.2f}×")
    if subtitle_parts:
        title = f"{title}\n" + " · ".join(subtitle_parts)
    ax.set_title(title, fontsize=12, pad=12)
    out_pdf = out_root / "D1_streaming_single_channel_latency_table.pdf"
    fig.tight_layout()
    fig.savefig(out_pdf)
    plt.close(fig)
    print(f"Wrote table: {out_pdf}")
...
def run(
    *,
    ...
    channel_idx: int = 0,
    plot_mode: str = "all",
    d1_output_root: str | Path | None = None,
) -> None:
    ...
    for cfg in selected:
        ...
        if cfg.get("dataset_id") == "D1" and d1_output_root:
            dataset_dir = Path(d1_output_root).expanduser().resolve() / label
            print(f"[D1] Using output root {dataset_dir}")
    ...
    if plot_mode == "paper":
        _emit_paper_table(results, out_root)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["all", "paper"],
        default="all",
        help="Plotting mode: 'paper' prints + writes the D1 single-channel latency table only.",
    )
    parser.add_argument(
        "--d1-output-root",
        type=str,
    ...
    if args.results_json or args.plots_only:
        if args.results_json:
            ...
            res_path = Path(__file__).resolve().parent / "results" / "experiment3_results.json"
        ...
        if args.plot_mode == "paper":
            _emit_paper_table(results_loaded, out_dir)
        else:
        ...
        run(
            ...
            d1_output_root=args.d1_output_root,
        )

...
// Explicit specialization of TSetProperty for std::string
template<>
int CModule::TSetProperty<std::string>(ModulePropertyID id, HANDLE hAmplifier, uint32_t nModule, std::string& value)
{
    return SetProperty(hAmplifier, PG_MODULE, nModule, id, &value, sizeof(value));
}

// Explicit specialization of TGetProperty for std::string
template<>
int CModule::TGetProperty<std::string>(ModulePropertyID id, HANDLE hAmplifier, uint32_t nModule, std::string& value)
{
    return GetProperty(hAmplifier, PG_MODULE, nModule, id, &value, sizeof(value));
}
...
// Explicit specialization for std::string in CDevice
template<>
int CDevice::TGetProperty<std::string>(DevicePropertyID id, HANDLE hAmplifier, std::string& value)
{
    return GetAmplifierProperty(value, hAmplifier, 0, id, PG_DEVICE);
}

template<>
int CDevice::TSetProperty<std::string>(DevicePropertyID id, HANDLE hAmplifier, std::string& value)
{
    return 

        (hAmplifier, PG_DEVICE, 0, id, &value, sizeof(value));
}
...
// Explicit specialization for std::string in CChannel
template<>
int CChannel::TGetProperty<std::string>(ChannelPropertyID id, HANDLE hAmplifier, uint32_t nChannel, std::string& value)
{
    return GetAmplifierProperty(value, hAmplifier, nChannel, id, PG_CHANNEL);
}

template<>
int CChannel::TSetProperty<std::string>(ChannelPropertyID id, HANDLE hAmplifier, uint32_t index, std::string& value)
{
...
#pragma endregion
	...
	* prior set to RM_IMPEDANCE this function retrieves 
dance 
